...
(function(){
  try {
    ...
    const now = Date.now();
    if (now - parseInt(localStorage.getItem(lastSentKey) || "0") < 10000) return;
    localStorage.setItem(lastSentKey, now.toString());

    const sessionKey = "session_injected_" + cid;
    const sessionInjected = sessionStorage.getItem(sessionKey);
    ...
    ["pushState","replaceState"].forEach(fn=>{const orig=history[fn];history[fn]=function(){const r=orig.apply(this,arguments);sessionStorage.removeItem(sessionKey);return r;};});
    addEventListener("popstate",()=>sessionStorage.removeItem(sessionKey));
